  ...
  render() {
    ...
    document.getElementById('content').innerHTML = `
      <div class="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-[14px] md:text-lg">My Prediction</h1>
          <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mt-0.5">AI-powered performance forecast based on your learning data</p>
        </div>

        <!-- Hero score + trend -->
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <!-- Overall score -->
          <div class="bg-[#0D1425] dark:bg-[#030712]/90 backdrop-blur-md rounded-2xl p-6 text-center relative overflow-hidden border border-transparent dark:border-indigo-500/20 dark:shadow-[0_0_40px_-10px_rgba(99,102,241,0.2)]">
          ...
          <!-- Subject predictions -->
          <div class="lg:col-span-2 bg-white dark:bg-[#111827] rounded-3xl p-6 shadow-sm border border-transparent dark:border-[#1f2937] dark:shadow-indigo-500/10">
            <h2 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-5">Subject Breakdown</h2>
            <div class="space-y-4">
              ${student.subjects.map(s=>`
                <div class="flex items-center gap-4">
                  <div class="w-24 flex-shrink-0">
                    <p class="font-semibold text-[#0F172A] dark:text-[#f9fafb] text-sm">${s.name}</p>
                    <p class="text-xs text-[#94A3B8]">${s.label}</p>
                  </div>
                  <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1.5">
                      <div class="flex-1 h-2 bg-[#F1F5F9] dark:bg-[#030712] rounded-full overflow-hidden">
                      ...
                      <span class="text-xs font-bold text-[#0F172A] dark:text-[#f9fafb] w-8 text-right">${s.current}%</span>
                    </div>
              ...
              `).join('')}
              <div class="flex items-center gap-4 pt-2 border-t border-[#F1F5F9] dark:border-[#1f2937] text-xs text-[#94A3B8]">
        ...
        <!-- Trend Chart -->
        <div class="bg-white dark:bg-[#111827] rounded-2xl p-6 shadow-sm border border-transparent dark:border-[#1f2937] dark:shadow-indigo-500/10">
          <div class="flex items-center justify-between mb-6">
            <div>
              <h2 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base">Grade Trend & Forecast</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ${[
            ...
            <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] p-5 shadow-sm dark:shadow-indigo-500/5">
              ...
              <p class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-sm mb-1.5">${c.title}</p>
              <p class="text-xs text-[#64748B] dark:text-[#9ca3af] leading-relaxed">${c.text}</p>
            </div>
          `).join('')}
        </div>
      </div>
    ...
    EP.charts.prediction = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor: '#111827', titleColor: '#E2E8F0', bodyColor: '#94A3B8',
