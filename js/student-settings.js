      ...
      <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-3">
            <button onclick="EP.actions.saveSettings()" class="bg-indigo-500 hover:bg-indigo-600 text-white text-[10px] font-bold uppercase tracking-widest px-6 py-3.5 rounded-xl shadow-lg shadow-indigo-500/20 active:scale-95 transition-all flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-1 space-y-6">
            <div class="bg-white rounded-2xl p-6 shadow-sm flex flex-col items-center text-center">
              <div class="relative mb-4 group cursor-pointer">
                <div class="w-24 h-24 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-700 p-1">
                   <div class="w-full h-full rounded-full bg-white flex items-center justify-center p-1 overflow-hidden shadow-inner">
                      ${avatarEl(currentUser.name, 'w-full h-full text-3xl font-black')}
                   </div>
                </div>
              ...
              <h2 class="font-display font-bold text-[#0F172A] text-xl tracking-tight">${currentUser.name}</h2>
              <p class="text-sm text-[#64748B] font-medium mt-0.5">${currentUser.program}</p>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm">
               <h3 class="font-display font-bold text-[#0F172A] text-sm mb-4">Academic Information</h3>
               <div class="space-y-5">
                  ${[
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0">
                           <p class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider mb-0.5">${f.label}</p>
          ...
          <div class="lg:col-span-2 space-y-6">
             ...
             <div class="bg-white rounded-2xl p-6 shadow-sm">
                <div class="flex items-center justify-between mb-6">
                   <h3 class="font-display font-bold text-[#0F172A] text-base flex items-center gap-2">
                     ...
                     { id: 'pred', title: 'Prediction Forecasts', desc: 'Weekly AI performance forecast summaries', on: false },
                      ...
                      <div class="flex items-center justify-between py-2 border-b border-[#F8FAFC] last:border-0 hover:bg-[#FDFDFF] transition-colors -mx-4 px-4 rounded-xl">
                         <div class="flex-1 pr-4">
                            <p class="text-sm font-bold text-[#0F172A]">${n.title}</p>
                            <p class="text-xs text-[#94A3B8] mt-0.5 leading-relaxed">${n.desc}</p>
                         </div>
             ...
             <!-- Appearance -->
             <div class="bg-white rounded-2xl p-6 shadow-sm">
                <h3 class="font-display font-bold text-[#0F172A] text-base mb-6 flex items-center gap-2">
                ...
                <div class="grid grid-cols-3 gap-3">
                    ${[
                      ...
                      <button onclick="EP.actions.setTheme('${t.id}')" class="flex-1 rounded-xl border-2 ${t.active?'border-indigo-400 shadow-lg shadow-indigo-500/10':'border-transparent opacity-60 hover:opacity-100'} ${t.bg} p-4 flex flex-col items-center gap-2 transition-all hover:scale-[1.02] active:scale-95 group shadow-sm">
                        ...
                        <span class="text-[10px] font-black ${t.active ? (t.id === 'light' ? 'text-indigo-600' : 'text-white') : 'text-slate-400'} tracking-widest uppercase italic transition-colors">${t.label}</span>
                      </button>
                    `).join('')}
                </div>
             </div>
             ...
             <div class="bg-rose-50 rounded-2xl border border-rose-100 p-6 flex items-center justify-between gap-4">
                <div class="flex items-start gap-4">
                   <div class="w-10 h-10 rounded-xl bg-rose-100 flex items-center justify-center flex-shrink-0">
                      ${EP.getIcon('exclamation-triangle', 'w-5 h-5 text-rose-600', 'solid')}
                   </div>
                   <div>
                      <h4 class="font-display font-bold text-rose-800 text-sm">Delete Account Data</h4>
                ...
                <button onclick="EP.actions.confirmDelete('All Account Data', () => { EP.notify('All local data cleared. Resetting...', 'error'); setTimeout(()=>location.reload(), 2000); })" class="px-4 py-2 bg-rose-600 text-white text-[10px] font-black rounded-xl hover:bg-rose-700 transition-colors uppercase tracking-widest shadow-sm shadow-rose-500/20 active:scale-95">
                   Delete Data
                </button>
             </div>
          </div>
