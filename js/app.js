...
function renderNav() {
  ...
  nav.innerHTML = items.map(item => {
    ...
    const activeClass = isActive ? "bg-indigo-500/15 text-indigo-500 active-nav" : "text-[#64748B] dark:text-[#9ca3af] hover:text-[#94A3B8] dark:hover:text-white hover:bg-white/5";
  ...
  if (mc) {
    mc.innerHTML = `
      <div class="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4" onclick="if(event.target===this)closeModal()">
        <div class="bg-white dark:bg-[#111827] rounded-[2rem] shadow-2xl w-full max-w-lg animate-[fadeUp_0.15s_ease-out] overflow-hidden border border-white/20 dark:border-white/5">
          <div class="max-h-[90vh] overflow-y-auto p-1.5 custom-scrollbar">
            <div class="bg-white dark:bg-[#111827] rounded-[1.6rem] overflow-hidden">
               ${html}
            </div>
          </div>
        </div>
...
function showPanel(html) {
  ...
  container.innerHTML = `
    <div id="panel-root" class="fixed inset-0 z-[60] overflow-hidden">
      ...
      <!-- Panel Body -->
      <div id="panel-body" class="absolute inset-y-0 right-0 bg-white dark:bg-[#111827] w-full max-w-lg shadow-2xl border-l border-[#E2E8F0] dark:border-[#1f2937] animate-[slideInRight_0.3s_cubic-bezier(0.4,0,0.2,1)_forwards] flex flex-col">
...
function getCalendarHTML(year, month) {
  ...
  return `
    <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] p-5 shadow-sm">
      <div class="flex items-center justify-between mb-4">
        <h3 class="font-display font-semibold text-[#0F172A] text-sm">${monthName} ${year}</h3>
        <div class="flex gap-1">
          <button onclick="changeMonth(-1)" class="p-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 text-[#94A3B8] transition-colors border border-transparent hover:border-[#E2E8F0] dark:hover:border-[#1f2937]">
        ...
        ${Array(daysInMonth).fill(0).map((_, i) => {
          ...
          return `
            <div class="h-8 flex flex-col items-center justify-center relative group cursor-pointer">
              <span class="text-xs font-medium ${isToday ? 'text-white bg-indigo-500 w-6 h-6 rounded-lg flex items-center justify-center shadow-sm shadow-indigo-500/20' : 'text-[#475569] dark:text-[#9ca3af] group-hover:text-indigo-600'}">${day}</span>
      ...
      <div class="flex gap-3">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center justify-between mb-0.5">
            <p class="text-[11px] font-bold text-[#0F172A] dark:text-[#f9fafb]">${n.title}</p>
            <span class="text-[9px] text-[#94A3B8]">${new Date(n.time).getHours()}h ago</span>
          </div>
          <p class="text-[10px] text-[#64748B] dark:text-[#9ca3af] leading-relaxed line-clamp-2">${n.text}</p>
        </div>
      </div>
    </div>
  `).join('');
...
EP.actions = {
  viewClassworkDetails(id) {
    ...
    showPanel(`
      <div class="p-0">
        <div class="p-8 bg-white dark:bg-[#111827] border-b border-[#F1F5F9] dark:border-[#1f2937] relative overflow-hidden">
          ...
          <div class="relative">
            <div class="flex items-center gap-2 mb-4">
              ...
              <span class="w-1 h-1 rounded-full bg-[#CBD5E1] dark:bg-[#1f2937]"></span>
              <span class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest">${cw.type}</span>
            </div>
            <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">${cw.title}</h3>
        ...
        <div class="p-8 space-y-5 pt-0 pb-5">
          ...
          <div>
            ...
            <div class="text-[#475569] dark:text-[#9ca3af] text-sm leading-relaxed space-y-4">
          ...
          <div>
            ...
            <div class="space-y-2">
              <div class="flex items-center justify-between p-3.5 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl hover:border-indigo-200 dark:hover:border-indigo-500/50 hover:bg-white dark:hover:bg-[#1f2937] transition-all cursor-pointer group">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="text-xs font-bold text-[#0F172A] dark:text-[#f9fafb]">ProblemSet_Instruction.pdf</p>
        ...
        <div class="p-6 py-5 bg-[#F8FAFC] dark:bg-[#030712] border-t border-[#F1F5F9] dark:border-[#1f2937] flex gap-3">
          <button onclick="closePanel()" class="flex-1 py-4 bg-white dark:bg-[#111827] border border-[#E2E8F0] dark:border-[#1f2937] hover:bg-gray-50 dark:hover:bg-white/5 text-[#475569] dark:text-[#9ca3af] font-bold text-[10px] uppercase tracking-widest rounded-xl transition-all active:scale-95">Close</button>
          ${cw.status !== 'Graded' ? `
  ...
  showFilterOptions(title='Data') {
    showModal(`
      <div class="p-4 md:p-5">
        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">Filter ${title}</h3>
        <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mb-8 leading-relaxed">Refine the view based on your specific criteria.</p>
        <div class="space-y-6 mb-8">
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              ...
              <button class="py-2.5 px-4 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] text-[#64748B] dark:text-[#9ca3af] text-xs font-bold rounded-xl hover:bg-white dark:hover:bg-[#1f2937] transition-all">Latest Activity</button>
            </div>
          </div>
          <div>
            ...
            <div class="flex flex-wrap gap-2">
              ${['All', 'High Risk', 'On Track', 'Graded'].map(f=>`
                <button class="px-4 py-2 border border-[#E2E8F0] dark:border-[#1f2937] text-[#475569] dark:text-[#9ca3af] text-xs font-semibold rounded-full hover:border-indigo-200 dark:hover:border-indigo-500/50 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all">${f}</button>
              `).join('')}
            </div>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <button onclick="closeModal()" class="py-4 bg-gray-50 dark:bg-[#030712] text-[#475569] dark:text-[#9ca3af] font-semibold rounded-xl text-[10px] uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Reset</button>
  ...
  joinClass() {
    showModal(`
      <div class="p-4 md:p-5 text-center">
        ...
        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">Join New Class</h3>
        <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mb-8 leading-relaxed">Enter your instructor's class code to enroll.</p>
        <input type="text" id="join-code" placeholder="CODE-2026" class="w-full px-6 py-4 rounded-xl border-2 border-[#F1F5F9] dark:border-[#1f2937] bg-transparent dark:text-white mb-8 focus:border-indigo-400 focus:ring-0 outline-none text-center text-xs font-bold tracking-widest uppercase transition-all">
        <div class="grid grid-cols-2 gap-3">
          <button onclick="closeModal()" class="py-4 bg-gray-50 dark:bg-[#030712] text-[#64748B] dark:text-[#9ca3af] font-bold rounded-xl text-[10px] uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Cancel</button>
  ...
  newClass() {
    showModal(`
      <div class="p-4 md:p-5">
        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">Create New Class</h3>
        <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mb-8">Set up a new learning environment.</p>
        <div class="space-y-4 mb-8">
          <div>
            <label class="block text-[10px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Class Name</label>
            <input type="text" id="new-class-name" placeholder="e.g. Advanced Web Design" class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium transition-all">
          </div>
          <div>
            <label class="block text-[10px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Class Code</label>
            <input type="text" id="new-class-code" placeholder="e.g. CS401" class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium tracking-wider uppercase transition-all">
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <button onclick="closeModal()" class="py-4 bg-gray-50 dark:bg-[#030712] text-[#475569] dark:text-[#9ca3af] font-bold rounded-xl text-[10px] uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Discard</button>
  ...
  newStudent() {
    showModal(`
      <div class="p-4 md:p-5">
        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">Enlist New Student</h3>
        <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mb-6 leading-relaxed">Add a student to the academic database for tracking and prediction.</p>
        <div class="space-y-4 mb-8">
          <div>
            <label class="block text-[10px] font-medium text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Full Name</label>
            <input type="text" id="new-student-name" placeholder="e.g. Juan De La Cruz" class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium transition-all">
          </div>
          <div>
            <label class="block text-[10px] font-medium text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Student ID/Email</label>
            <input type="text" id="new-student-id" placeholder="e.g. 2024-1001 or email" class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium transition-all">
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <button onclick="closeModal()" class="py-4 bg-gray-50 dark:bg-[#030712] text-[#475569] dark:text-[#9ca3af] font-bold rounded-xl text-[10px] uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Discard</button>
  ...
  postAnnouncement(classTitle) {
    showModal(`
      <div class="p-4 md:p-5">
        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">New Announcement</h3>
        <p class="text-[#64748B] dark:text-[#9ca3af] text-sm mb-8 leading-relaxed">Share an important update with ${classTitle}.</p>
        <div class="space-y-4 mb-8">
          <div>
            <label class="block text-[10px] font-medium text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Announcement Title</label>
            <input type="text" id="ann-title" placeholder="e.g. Schedule Change" class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium transition-all">
          </div>
          <div>
            <label class="block text-[10px] font-medium text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Message Content</label>
            <textarea id="ann-text" rows="4" placeholder="Type your message here..." class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium resize-none transition-all"></textarea>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <button onclick="closeModal()" class="py-4 bg-gray-50 dark:bg-[#030712] text-[#475569] dark:text-[#9ca3af] font-bold rounded-xl text-[10px] uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Discard</button>
  ...
  newAssignment() {
    showModal(`
      <div class="p-4 md:p-5">
        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">Post New Assignment</h3>
        <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mb-8 leading-relaxed">Create a new task for your students to complete.</p>
        <div class="space-y-4 mb-8">
           <div>
              <label class="block text-[10px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Title</label>
              <input type="text" placeholder="e.g. Introduction to React" class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium transition-all">
           </div>
           <div class="grid grid-cols-2 gap-3">
              <div>
                 <label class="block text-[10px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Due Date</label>
                 <input type="date" class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium transition-all">
              </div>
              <div>
                 <label class="block text-[10px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-1.5 ml-1">Max Score</label>
                 <input type="number" placeholder="100" class="w-full px-4 py-3 bg-[#F8FAFC] dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] rounded-xl outline-none focus:border-indigo-400 dark:text-white text-sm font-medium transition-all">
              </div>
           </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <button onclick="closeModal()" class="py-3.5 bg-gray-50 dark:bg-[#030712] text-[#475569] dark:text-[#9ca3af] font-semibold rounded-xl text-[10px] uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Discard</button>
  ...
  gradeSubmissions(title) {
    showModal(`
      <div class="p-4 md:p-5">
        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">Grade Submissions</h3>
        <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mb-6">${title}</p>
        <div class="space-y-3 mb-8">
           ${['Maria Santos', 'Juan Dela Cruz', 'Elena Gilbert'].map(name=>`
              <div class="flex items-center justify-between p-3 bg-[#F8FAFC] dark:bg-[#030712] rounded-xl border border-[#E2E8F0] dark:border-[#1f2937]">
                 <div class="flex items-center gap-3">
                    ${avatarEl(name, 'w-8 h-8 text-[10px]')}
                    <p class="text-xs font-bold text-[#475569] dark:text-[#f9fafb]">${name}</p>
                 </div>
                 <input type="number" placeholder="0" class="w-16 px-2 py-1.5 bg-white dark:bg-[#111827] border border-[#E2E8F0] dark:border-[#1f2937] rounded-lg text-xs font-bold text-center focus:border-indigo-400 dark:text-white outline-none transition-all">
              </div>
           `).join('')}
        </div>
  ...
  confirmDelete(title, callback) {
    showModal(`
      <div class="p-4 md:p-5 text-center">
        ...
        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base mb-2">Delete ${title}?</h3>
        <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mb-10 leading-relaxed">This action is irreversible for the current session. All related student data will be purged.</p>
        <div class="flex gap-3">
          <button onclick="closeModal()" class="flex-1 py-4 bg-gray-50 dark:bg-[#030712] text-[#475569] dark:text-[#9ca3af] font-bold rounded-xl text-[10px] uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Cancel</button>
  ...
  viewDetails(id) {
    ...
    showPanel(`
      <div class="flex flex-col h-full bg-[#F8FAFC] dark:bg-[#030712]">
        <!-- Hero Header -->
        <div class="px-8 pt-8 pb-9 bg-white dark:bg-[#111827] relative overflow-hidden border-b border-[#E2E8F0] dark:border-[#1f2937]">
          <div class="absolute -right-20 -top-20 w-64 h-64 bg-indigo-500/5 rounded-full blur-3xl"></div>
          <div class="relative flex items-start justify-between">
            <div class="flex items-center gap-5">
              <div class="relative">
                ${avatarEl(s.name, 'w-20 h-20 text-2xl font-semibold ring-4 ring-white dark:ring-[#1f2937] shadow-xl shadow-indigo-500/10')}
                <div class="absolute -bottom-1 -right-1 w-7 h-7 bg-emerald-500 rounded-full border-4 border-white dark:border-[#111827] flex items-center justify-center">
                  ${EP.getIcon('check', 'w-3 h-3 text-white', 'solid')}
                </div>
              </div>
              <div>
                <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-xl leading-tight">${s.name}</h3>
                <p class="text-[#64748B] dark:text-[#9ca3af] text-xs font-medium mt-1">${s.email}</p>
            ...
            <button onclick="closePanel()" class="p-2.5 rounded-xl bg-[#F8FAFC] dark:bg-[#030712] text-[#94A3B8] hover:text-indigo-600 transition-all active:scale-95">
        ...
        <div class="flex-1 overflow-y-auto p-8 space-y-8 scroll-smooth custom-scrollbar">
          ...
          <div class="grid grid-cols-3 gap-3">
             <div class="bg-white dark:bg-[#111827] p-4 rounded-3xl border border-[#E2E8F0] dark:border-[#1f2937] shadow-sm transform transition-all hover:scale-105">
                <p class="text-[9px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-2 text-center">Current GPA</p>
                <p class="text-xl font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-center">${s.gpa.toFixed(2)}</p>
             </div>
             <div class="bg-white dark:bg-[#111827] p-4 rounded-3xl border border-[#E2E8F0] dark:border-[#1f2937] shadow-sm transform transition-all hover:scale-105">
                <p class="text-[9px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-2 text-center">Attendance</p>
                <p class="text-xl font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-center">${s.attendance}%</p>
             </div>
             <div class="bg-white dark:bg-[#111827] p-4 rounded-3xl border border-[#E2E8F0] dark:border-[#1f2937] shadow-sm transform transition-all hover:scale-105">
          ...
          <!-- Grade Trend Map -->
          <div class="bg-white dark:bg-[#111827] p-6 rounded-3xl border border-[#E2E8F0] dark:border-[#1f2937] shadow-sm">
            <div class="flex items-center justify-between mb-6">
              <h4 class="text-[10px] font-semibold text-[#0F172A] dark:text-[#f9fafb] uppercase tracking-widest flex items-center gap-2">
          ...
          <div class="bg-[#0F172A] dark:bg-[#1f2937] p-6 rounded-3xl shadow-xl relative overflow-hidden group">
          ...
          <div class="space-y-4">
             ...
             <div class="grid grid-cols-2 gap-3">
                <button onclick="EP.notify('Meeting request sent via EduPredict Hub', 'success')" class="p-4 bg-white dark:bg-[#111827] border border-[#E2E8F0] dark:border-[#1f2937] rounded-2xl flex flex-col items-center gap-2 hover:border-indigo-300 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-all group active:scale-95 shadow-sm">
                   <div class="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-500 group-hover:bg-white dark:group-hover:bg-[#111827] transition-all">
                      ${EP.getIcon('calendar-days', 'w-5 h-5')}
                   </div>
                   <span class="text-[10px] font-bold text-[#475569] dark:text-[#9ca3af] uppercase tracking-wider">Schedule Sync</span>
                </button>
                <button onclick="EP.notify('Automated study guide sent to student', 'success')" class="p-4 bg-white dark:bg-[#111827] border border-[#E2E8F0] dark:border-[#1f2937] rounded-2xl flex flex-col items-center gap-2 hover:border-amber-300 dark:hover:border-amber-500 hover:bg-amber-50 dark:hover:bg-amber-500/10 transition-all group active:scale-95 shadow-sm">
                   <div class="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-500 group-hover:bg-white dark:group-hover:bg-[#111827] transition-all">
                      ${EP.getIcon('light-bulb', 'w-5 h-5')}
                   </div>
                   <span class="text-[10px] font-bold text-[#475569] dark:text-[#9ca3af] uppercase tracking-wider">Send Nudge</span>
                </button>
             </div>
             <button onclick="EP.notify('Full academic audit generated', 'info')" class="w-full py-4 bg-[#F1F5F9] dark:bg-[#030712] text-[#475569] dark:text-[#9ca3af] font-semibold text-[10px] uppercase tracking-widest rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] hover:bg-white dark:hover:bg-[#111827] transition-all active:scale-95 shadow-sm">
