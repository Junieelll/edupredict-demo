      ...
      <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 class="font-display font-black text-[#0F172A] text-xl leading-none">${cls.name}</h1>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 space-y-6">
            <!-- Hero Card -->
            <div class="bg-white rounded-3xl border border-[#E2E8F0] overflow-hidden shadow-sm relative">
              <div class="h-40 bg-gradient-to-br ${classGradient(cls.color)} relative overflow-hidden">
                 <div class="absolute inset-0 bg-black/10"></div>
                 <div class="absolute -right-8 -bottom-8 w-48 h-48 bg-white/10 rounded-full blur-3xl"></div>
                 <div class="absolute top-6 left-8">
                   <span class="px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-[10px] font-black text-white uppercase tracking-widest border border-white/20">${cls.code}</span>
                 </div>
              </div>
              <div class="px-8 pb-8 -mt-12 relative">
                <div class="bg-white rounded-2xl border border-[#E2E8F0] p-6 shadow-xl shadow-black/5 grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div class="text-center">
                    <p class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1">Students</p>
                    <p class="font-display font-black text-[#0F172A] text-lg">${cls.students}</p>
                  </div>
                  <div class="text-center border-l border-[#F1F5F9]">
                    <p class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1">Your Grade</p>
                    <p class="font-display font-black text-indigo-600 text-lg">${avgScore}${avgScore !== '—' ? '%' : ''}</p>
                  </div>
                  <div class="text-center border-l border-[#F1F5F9]">
                    <p class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1">Rank</p>
                    <p class="font-display font-black text-[#0F172A] text-lg">Top 15%</p>
                  </div>
                  <div class="text-center border-l border-[#F1F5F9]">
                    <p class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1">Next Class</p>
                    <p class="font-display font-black text-amber-500 text-lg">${cls.nextClass.split(',')[0]}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div class="bg-white rounded-3xl border border-[#E2E8F0] p-6 shadow-sm">
                 <h3 class="font-display font-bold text-[#0F172A] text-sm mb-4 flex items-center gap-2">
                 ...
                 <div class="mt-6 space-y-3">
                    <div class="flex items-center gap-3 p-3 bg-[#F8FAFC] rounded-xl border border-[#F1F5F9]">
                       ...
                       <div>
                          <p class="text-[10px] font-bold text-[#94A3B8] uppercase">Schedule</p>
                    ...
                    <div class="flex items-center gap-3 p-3 bg-[#F8FAFC] rounded-xl border border-[#F1F5F9]">
                       ...
                       <div>
                          <p class="text-[10px] font-bold text-[#94A3B8] uppercase">Location</p>
                   ...
                   ${announcements.length ? announcements.map(a => `
                     <div class="p-4 bg-amber-50/50 border border-amber-100 rounded-2xl relative overflow-hidden group">
                       ...
                       <div class="flex justify-between items-start mb-1">
                          <p class="text-xs font-bold text-amber-800">${a.title}</p>
            ...
            <div class="bg-white rounded-3xl border border-[#E2E8F0] overflow-hidden shadow-sm">
               <div class="px-6 py-5 border-b border-[#F1F5F9] flex items-center justify-between">
                 <h3 class="font-display font-bold text-[#0F172A] text-sm flex items-center gap-2">
                    ${EP.getIcon('clipboard-document-list', 'w-5 h-5 text-emerald-500', 'solid')} Classwork & Submissions
                 </h3>
                 <button onclick="navigate('classwork')" class="text-[10px] font-black text-indigo-500 uppercase tracking-widest hover:text-indigo-600">View All</button>
               </div>
                    ...
                    <div class="p-6 flex items-center justify-between hover:bg-[#FDFDFF] transition-colors cursor-pointer group" onclick="EP.actions.viewClassworkDetails(${cw.id})">
                       <div class="flex items-center gap-4">
                          <div class="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center text-[#94A3B8] group-hover:bg-indigo-50 group-hover:text-indigo-500 transition-all">
                             ${EP.getIcon(typeIcon(cw.type), 'w-6 h-6')}
                          </div>
                          <div>
                             <p class="text-sm font-bold text-[#0F172A] mb-0.5">${cw.title}</p>
            ...
            <div class="bg-white rounded-3xl border border-[#E2E8F0] p-6 shadow-sm text-center">
               <div class="w-20 h-20 rounded-full bg-indigo-500 mx-auto mb-4 p-1">
                  <div class="w-full h-full rounded-full border-4 border-white overflow-hidden bg-[#F8FAFC] flex items-center justify-center">
                     ${avatarEl(cls.instructor, 'w-full h-full text-2xl font-black')}
                  </div>
               </div>
               <h4 class="font-display font-bold text-[#0F172A] text-base">${cls.instructor}</h4>
               <p class="text-xs text-[#94A3B8] font-medium mb-4">Course Instructor</p>
               <div class="pt-4 border-t border-[#F1F5F9] flex gap-2">
                  <button class="flex-1 py-3.5 bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-indigo-100 transition-all active:scale-95">Message</button>
                  <button class="flex-1 py-3.5 bg-[#F8FAFC] text-[#475569] text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#F1F5F9] transition-all border border-[#E2E8F0] active:scale-95">Profile</button>
               </div>
            </div>

            <!-- Grade Breakdown -->
            <div class="bg-white rounded-3xl border border-[#E2E8F0] p-6 shadow-sm">
               <h3 class="font-display font-bold text-[#0F172A] text-sm mb-6 flex items-center gap-2">
                  ...
                  ${[
                    ...
                    <div class="group">
                       <div class="flex justify-between items-center mb-1.5">
                          <span class="text-[10px] font-bold text-[#475569] uppercase tracking-wider">${w.label}</span>
                          <span class="text-xs font-black text-${w.color}-500">${w.weight}%</span>
                       </div>
