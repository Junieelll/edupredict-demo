    ...
    document.getElementById('content').innerHTML = `
      <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex items-center gap-2 bg-indigo-50 border shrink-0 border-indigo-100 rounded-xl px-3 py-2 md:px-4 md:py-2.5 shadow-sm">
            ${EP.getIcon('sparkles', 'w-4 h-4 text-indigo-500')}
            <span class="text-[10px] md:text-xs font-semibold text-indigo-600 uppercase tracking-widest md:normal-case md:tracking-normal">AI-Powered</span>
          </div>
        </div>
          ...
          <div class="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden shadow-sm">
            ...
            <div class="divide-y divide-[#F1F5F9]">
              ${group.items.map(item => {
                ...
                return `
                  <div class="p-5 flex flex-col sm:flex-row items-start gap-4 hover:bg-[#F8FAFC] transition-colors cursor-pointer group/item">
                    ...
                    <div class="flex-1 min-w-0">
                      <p class="font-semibold text-[#0F172A] text-sm group-hover/item:text-indigo-600 transition-colors">${item.title}</p>
                    ...
                    <div class="flex flex-row sm:flex-col items-center sm:items-end gap-2 flex-shrink-0 w-full sm:w-auto mt-3 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-[#F1F5F9]">
                      <span class="text-[9px] md:text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full border ${diffConfig[item.difficulty]||'bg-gray-100 text-gray-600'}">${item.difficulty}</span>
