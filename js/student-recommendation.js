    ...
    document.getElementById('content').innerHTML = `
      <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-start justify-between gap-4">
          <div>
            <h1 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-[14px] md:text-lg">AI Recommendations</h1>
            <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mt-0.5">Personalized study materials based on your performance</p>
          </div>
        ...
        <div class="bg-gradient-to-r from-indigo-50 to-violet-50 dark:from-indigo-500/10 dark:to-violet-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-2xl p-5 flex items-start gap-4">
          ...
          <div>
            <p class="font-semibold text-[#0F172A] dark:text-[#f9fafb] text-sm">How recommendations work</p>
            <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mt-1 leading-relaxed">EduPredict analyzes your quiz scores, attendance, assignment submissions, and learning patterns to surface the most relevant study materials for you right now.</p>
          </div>
        </div>
        ...
        ${recommendations.map((group, gi) => {
          ...
          return `
          <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] overflow-hidden shadow-sm">
            <!-- Group header -->
              ...
              ${group.items.map(item => {
                ...
                return `
                  <div class="p-5 flex flex-col sm:flex-row items-start gap-4 hover:bg-[#F8FAFC] dark:hover:bg-white/5 transition-colors cursor-pointer group/item">
                    ...
                    <div class="flex-1 min-w-0">
                      <p class="font-semibold text-[#0F172A] dark:text-[#f9fafb] text-sm group-hover/item:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">${item.title}</p>
                    ...
                    <div class="flex flex-row sm:flex-col items-center sm:items-end gap-2 flex-shrink-0 w-full sm:w-auto mt-3 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-[#F1F5F9] dark:border-[#1f2937]">
            ...
            <div class="p-4 bg-[#F8FAFC] dark:bg-[#030712] border-t border-[#F1F5F9] dark:border-[#1f2937]">
