    ...
    document.getElementById('content').innerHTML = `
      <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 class="font-display font-semibold text-[#0F172A] dark:text-[#F8FAFC] text-sm md:text-lg">Notifications</h1>
            <p class="text-[#64748B] dark:text-[#94A3B8] text-xs mt-0.5">Stay updated with your latest academic alerts and system updates.</p>
          </div>
        ...
        <div class="space-y-8">
          ${Object.keys(grouped).length === 0 ? `
            <div class="bg-white dark:bg-[#141D33] rounded-3xl border border-[#E2E8F0] dark:border-[#1E293B] p-12 text-center shadow-sm dark:shadow-indigo-500/10">
                <div class="w-16 h-16 bg-[#F8FAFC] dark:bg-[#0D1425] rounded-2xl flex items-center justify-center mx-auto mb-4 text-[#CBD5E1] dark:text-[#475569]">
                ${EP.getIcon('bell-slash', 'w-8 h-8')}
              </div>
              <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#F8FAFC]">No notifications found</h3>
              <p class="text-[#64748B] dark:text-[#94A3B8] text-xs mt-1">We couldn't find any alerts matching your search.</p>
            </div>
          ` : Object.entries(grouped).map(([date, items]) => `
            <div>
              <h3 class="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-4 ml-1">${date}</h3>
              <div class="bg-white dark:bg-[#141D33] rounded-3xl border border-[#E2E8F0] dark:border-[#1E293B] overflow-hidden shadow-sm divide-y divide-[#F1F5F9] dark:divide-[#1E293B] dark:shadow-indigo-500/10">
                ${items.map(n => `
