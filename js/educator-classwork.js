    ...
    document.getElementById('content').innerHTML = `
      <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between gap-4">
          <div>
            <h1 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-[14px] md:text-lg">Classwork Management</h1>
            <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mt-0.5">Track and grade ${classworks.length} active assignments</p>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ${[
            ...
            <div class="bg-white dark:bg-[#111827] rounded-2xl p-5 shadow-sm dark:shadow-indigo-500/5 border border-transparent dark:border-[#1f2937] flex items-center gap-4">
        ...
        <div class="grid grid-cols-1 gap-4">
          ${classworks.map(cw => {
            ...
            return `
              <div class="bg-white dark:bg-[#111827] rounded-2xl border border-transparent dark:border-[#1f2937] p-5 shadow-sm dark:shadow-indigo-500/5 hover:border-indigo-200 dark:hover:border-indigo-500/50 transition-all group">
                <div class="flex flex-col md:flex-row md:items-center gap-5">
                  <div class="w-12 h-12 rounded-xl bg-gray-50 dark:bg-[#030712] flex items-center justify-center flex-shrink-0">
                    ${typeIcon(cw.type, 'w-6 h-6 text-indigo-500 dark:text-indigo-400')}
                  </div>
                  <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-4 mb-2">
                      <div>
                        <h3 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-sm group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">${cw.title}</h3>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center gap-5 mt-6 pt-5 border-t border-[#F8FAFC] dark:border-[#1f2937]">
                      <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-center mb-1.5 text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider">
                          <span>Submissions</span>
                          <span class="text-[#475569] dark:text-[#f9fafb]">${cw.submissions}/${cw.total} (${subPct}%)</span>
                        </div>
                        <div class="h-1.5 bg-[#F1F5F9] dark:bg-[#030712] rounded-full overflow-hidden border border-transparent dark:border-[#1f2937]">
                      ...
                      <div class="grid grid-cols-2 sm:flex items-center gap-4 sm:gap-8">
                        <div class="min-w-0">
                          ...
                          <p class="text-xs font-semibold text-[#475569] dark:text-[#f9fafb] mt-0.5">${new Date(cw.dueDate).toLocaleDateString()}</p>
                        </div>
                        <div class="min-w-0 border-l border-[#F1F5F9] dark:border-[#1f2937] pl-4 sm:border-l-0 sm:pl-0 sm:text-center">
                          <p class="text-[10px] font-bold text-[#94A3B8] uppercase leading-tight italic tracking-wide">Graded</p>
                          <p class="text-xs font-semibold text-[#475569] dark:text-[#f9fafb] mt-0.5">${cw.status === 'Graded' ? cw.submissions : '0'}/${cw.submissions}</p>
                        </div>
                      </div>
                      ...
                      <div class="flex items-center gap-2 pt-2 lg:pt-0 lg:ml-auto">
                        <div class="flex gap-2.5">
                          ...
                          <button onclick="EP.notify('Assignment editor not available', 'info')" class="w-10 h-10 flex items-center justify-center bg-white dark:bg-[#030712] border border-[#E2E8F0] dark:border-[#1f2937] text-[#64748B] dark:text-[#9ca3af] hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-500/50 rounded-xl transition-all hover:shadow-sm active:scale-95">
