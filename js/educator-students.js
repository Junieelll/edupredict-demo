    ...
    document.getElementById('content').innerHTML = `
      <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-[14px] md:text-lg">Students</h1>
            <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mt-0.5">Track and manage ${students.length} students across all courses</p>
          </div>
          <div class="flex items-center gap-2 flex-wrap md:flex-nowrap">
             <div class="relative flex-1 sm:w-64">
                ...
                <input type="text" placeholder="Search students..." class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-[#E2E8F0] dark:border-[#1f2937] bg-white dark:bg-[#111827] text-[#0F172A] dark:text-[#f9fafb] text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 transition-all shadow-sm placeholder-[#94A3B8]">
             </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ${[
            ...
            <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] p-4 shadow-sm dark:shadow-indigo-500/5 flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-base font-display font-bold text-[#0F172A] dark:text-[#f9fafb]">${s.value}</p>
              </div>
            </div>
          `).join('')}
        </div>
        ...
        <!-- Student List -->
        <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] shadow-sm dark:shadow-[0_8px_30px_rgb(0,0,0,0.12)] overflow-hidden">
          <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="bg-[#F8FAFC] dark:bg-[#030712] border-b border-[#E2E8F0] dark:border-[#1f2937] text-[11px] font-bold text-[#64748B] dark:text-[#9ca3af] uppercase tracking-wider">
                  ...
                  <tr class="hover:bg-[#F8FAFC] dark:hover:bg-white/5 transition-colors group">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          <p class="text-sm font-semibold text-[#0F172A] dark:text-[#f9fafb] group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">${s.name}</p>
                    ...
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-2">
                        <div class="flex-1 w-16 h-1.5 bg-[#F1F5F9] dark:bg-[#030712] rounded-full overflow-hidden border border-transparent dark:border-[#1f2937]">
                          <div class="h-full rounded-full bg-indigo-500" style="width: ${s.gpa * 25}%"></div>
                        </div>
                        <span class="text-sm font-bold text-[#475569] dark:text-[#f9fafb]">${s.gpa}</span>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-1.5 text-sm font-medium text-[#475569] dark:text-[#9ca3af]">
                    ...
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                         <button onclick="EP.notify('Email composer not available in prototype', 'info')" class="p-2 border border-[#E2E8F0] dark:border-[#1f2937] rounded-lg text-[#64748B] dark:text-[#9ca3af] hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-500/50 hover:bg-white dark:hover:bg-white/5 transition-all shadow-sm">
                            ${EP.getIcon('envelope', 'w-4 h-4')}
                         </button>
                         <button onclick="EP.actions.viewDetails(${s.id})" class="p-2 border border-[#E2E8F0] dark:border-[#1f2937] rounded-lg text-[#64748B] dark:text-[#9ca3af] hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-500/50 hover:bg-white dark:hover:bg-white/5 transition-all shadow-sm">
