    ...
    document.getElementById('content').innerHTML = `
      <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between gap-4">
          <div>
            <h1 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-[14px] md:text-lg">Educator Dashboard</h1>
            <p class="text-[#64748B] dark:text-[#9ca3af] text-xs mt-0.5">2nd Semester, AY 2025-2026 · ${new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'})}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ${[
            ...
            <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] p-5 shadow-sm dark:shadow-indigo-500/10 hover-lift relative overflow-hidden group">
              ...
              <div class="relative">
                ...
                <p class="font-display font-bold text-[#0F172A] dark:text-[#f9fafb] text-2xl tracking-tight">${s.value}</p>
                <div class="flex flex-col mt-1">
                  <p class="text-xs font-semibold text-[#475569] dark:text-[#f9fafb]">${s.label}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <!-- Chart: Class Average -->
          <div class="lg:col-span-2 bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] p-6 shadow-sm dark:shadow-indigo-500/10">
            <div class="flex items-center justify-between mb-5">
              <div>
                <h2 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base">Class Performance</h2>
                <p class="text-xs text-[#94A3B8] mt-0.5">Average scores over the semester</p>
              </div>
              <select id="class-selector" onchange="EP.educatorDashboard.updateChart()" class="text-xs font-semibold text-[#475569] dark:text-[#9ca3af] border border-[#E2E8F0] dark:border-[#1f2937] bg-[#F8FAFC] dark:bg-[#030712] rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500/20">
          ...
          <!-- Recent Classwork -->
          <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] p-5 shadow-sm dark:shadow-indigo-500/10">
            <div class="flex items-center justify-between mb-4">
              <h2 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-sm">Recent Classwork</h2>
                ...
                return `
                  <div class="group">
                    <div class="flex items-start gap-2.5">
                      <div class="w-7 h-7 rounded-lg bg-gray-50 dark:bg-[#030712] flex items-center justify-center flex-shrink-0 mt-0.5">
                        ${typeIcon(cw.type, 'w-4 h-4')}
                      </div>
                      <div class="flex-1 min-w-0">
                        <p class="text-xs font-semibold text-[#0F172A] dark:text-[#f9fafb] truncate">${cw.title}</p>
                        <p class="text-[10px] text-[#94A3B8]">${cls?.code||''} · ${cw.submissions}/${cw.total} submitted</p>
                        <div class="mt-1.5 h-1 bg-[#F1F5F9] dark:bg-[#030712] rounded-full overflow-hidden">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <!-- At-Risk Students -->
          <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] shadow-sm dark:shadow-indigo-500/10 overflow-hidden">
            <div class="px-5 py-4 border-b border-[#F1F5F9] dark:border-[#1f2937] flex items-center justify-between">
              <h2 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base">Students Needing Attention</h2>
            ...
            <div class="divide-y divide-[#F8FAFC] dark:divide-[#1f2937]">
              ${students.filter(s=>s.dropoutRisk!=='Low').slice(0,5).map(s=>`
                <div class="px-5 py-3.5 flex items-center gap-3 hover:bg-[#F8FAFC] dark:hover:bg-white/5 transition-colors">
                  ${avatarEl(s.name)}
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-semibold text-[#0F172A] dark:text-[#f9fafb] truncate">${s.name}</p>
          ...
          <div class="bg-white dark:bg-[#111827] rounded-2xl border border-[#E2E8F0] dark:border-[#1f2937] shadow-sm dark:shadow-indigo-500/10 overflow-hidden">
            <div class="px-5 py-4 border-b border-[#F1F5F9] dark:border-[#1f2937] flex items-center justify-between">
              <h2 class="font-display font-semibold text-[#0F172A] dark:text-[#f9fafb] text-base">My Classes</h2>
            ...
            <div class="divide-y divide-[#F8FAFC] dark:divide-[#1f2937]">
              ${classes.map(cls=>`
                <div onclick="navigate('classes')" class="px-5 py-3.5 flex items-center gap-3 hover:bg-[#F8FAFC] dark:hover:bg-white/5 transition-colors cursor-pointer">
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-semibold text-[#0F172A] dark:text-[#f9fafb] truncate">${cls.name}</p>
                    <p class="text-xs text-[#94A3B8]">${cls.students} students · ${cls.schedule}</p>
                  </div>
                  <div class="text-right flex-shrink-0">
                    <p class="text-sm font-bold text-[#0F172A] dark:text-[#f9fafb]">${cls.avgScore}%</p>
    ...
    EP.charts.classChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { backgroundColor:'#111827', titleColor:'#E2E8F0', bodyColor:'#94A3B8', borderColor:'rgba(255,255,255,0.1)', borderWidth:1, cornerRadius:12, padding:10 }
        },
        scales: {
